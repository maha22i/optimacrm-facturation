...
const EDITABLE_STATUTS: StatutDevis[] = ['BROUILLON', 'ENVOYE'];
...
function ligneFromApi(l: DevisLigne): LigneForm {
  return {
    _uid: uid(),
    type: l.type,
    ...
    showDescription: !!l.description_detaillee,
  };
}

function champFromApi(c: DevisChamp): ChampForm {
  return {
    _uid: uid(),
    cle: c.cle,
    label: c.label,
...
export default function ModifierDevisPage() {
  ...
  useEffect(() => {
    ...
    async function loadDevis() {
      ...
      try {
        ...
        if (!EDITABLE_STATUTS.includes(d.statut)) {
          setReadOnly(true);
          setStatut(d.statut);
          setNumeroDevis(d.numero_devis);
          setLoading(false);
          setTimeout(() => router.replace(`/dashboard/devis/${devisId}`), 2500);
          return;
        }
        ...
        setModePaiement(d.mode_paiement);
        ...
        if (cancelled) return;
        setLoadError(err instanceof Error ? err.message : 'Erreur lors du chargement du devis');
      } finally {
  ...
  const handleSave = useCallback(async (silent = false) => {
    ...
    if (!objet.trim()) {
      if (!silent) setToast({ message: 'Veuillez renseigner l\'objet du devis', type: 'error' });
      return;
    }
    ...
    try {
      ...
      await api.put<ApiResponse<Devis>>(`/devis/${devisId}`, payload);
      setDirty(false);
      setLastSaved(new Date());
      if (!silent) setToast({ message: 'Devis enregistré avec succès', type: 'success' });
    } catch (err) {
  ...
  const handleEnvoyer = useCallback(async () => {
    ...
    try {
      const payload = buildPayload();
      await api.put<ApiResponse<Devis>>(`/devis/${devisId}`, payload);
      await api.post<ApiResponse<Devis>>(`/devis/${devisId}/envoyer`, {});
      setStatut('ENVOYE');
      setDirty(false);
      setLastSaved(new Date());
      setToast({ message: 'Devis envoyé au client avec succès', type: 'success' });
    } catch (err) {
  ...
  // ---------------------------------------------------------------------------
  // États de chargement / erreur / read-only
  // ---------------------------------------------------------------------------
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          <div className="animate-spin h-8 w-8 border-[3px] border-blue-600 border-t-transparent rounded-full mx-auto" />
          <p className="mt-4 text-sm text-gray-500">Chargement du devis...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className={`${cardClass} p-8 max-w-md text-center`}>
          <div className="h-12 w-12 rounded-full bg-amber-50 flex items-center justify-center mx-auto mb-4">
            <svg className="h-6 w-6 text-amber-500" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 1 0-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-6.75a2.25 2.25 0 0 0-2.25-2.25H6.75a2.25 2.25 0 0 0-2.25 2.25v6.75a2.25 2.25 0 0 0 2.25 2.25Z" /></svg>
          </div>
          <h2 className="text-lg font-bold text-gray-900 mb-2">Devis non modifiable</h2>
          <p className="text-sm text-gray-500 mb-2">
            Ce devis ne peut plus être modifié (statut : <span className="font-semibold">{STATUT_LABELS[statut]}</span>)
          </p>
          <p className="text-xs text-gray-400 mb-6">{numeroDevis}</p>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          ...
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-3">
            ...
            <span>
              Modifier le devis <span className="text-blue-600">{numeroDevis}</span>
            </span>
          </h1>
          <div className="mt-2 flex items-center gap-2">
            <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-1 rounded-full ${
              statut === 'BROUILLON' ? 'bg-gray-100 text-gray-600' :
              statut === 'ENVOYE' ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-600'
            }`}>
              <span className={`h-1.5 w-1.5 rounded-full ${
                statut === 'BROUILLON' ? 'bg-gray-400' :
                statut === 'ENVOYE' ? 'bg-blue-500' : 'bg-gray-400'
              }`} />
            ...
            <div className={`${cardClass} p-6 space-y-3`}>
              ...
              {statut === 'BROUILLON' && (
                <button
                  ...
                  className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-emerald-500/25 hover:shadow-emerald-500/40 hover:from-emerald-600 hover:to-teal-700 disabled:opacity-50 transition-all cursor-pointer"
                >
                  {sending ? (
                    <div className="animate-spin h-4 w-4 border-2 border-white border-t-transparent rounded-full" />
                  ) : (
