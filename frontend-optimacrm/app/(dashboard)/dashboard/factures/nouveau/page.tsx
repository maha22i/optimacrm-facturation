...
import type { Client, ApiResponse, PaginatedResponse, FactureLigne, TypeLigneFacture } from '@/lib/types';
...
export default function NouvelleFacturePage() {
  ...
  const selectClient = (c: Client) => {
    ...
    setForm(f => ({
      ...f,
      mode_reglement: c.mode_paiement_prefere || 'Prélèvement',
      site_concerne_nom: c.raison_sociale,
      site_concerne_email: c.email_principal || '',
    }));
  };
  ...
  const addLigne = (type: TypeLigneFacture = 'PRODUIT') => {
    setLignes(prev => [...prev, emptyLigne(type)]);
  };
  ...
  const handleSave = async (validate: boolean) => {
    ...
    try {
      ...
      const res = await api.post<ApiResponse<{ id: number }>>('/factures', body);
      if (validate) {
        await api.post(`/factures/${res.data.id}/valider`, {});
      }
      router.push(`/dashboard/factures/${res.data.id}`);
    } catch (err: unknown) {
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="date" value={form.date_creation} onChange={e => setForm(f => ({ ...f, date_creation: e.target.value }))}
                  className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-violet-400 focus:ring-2 focus:ring-violet-500/10 outline-none" />
              </div>
              <div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <input type="text" value={form.site_concerne_nom} onChange={e => setForm(f => ({ ...f, site_concerne_nom: e.target.value }))} placeholder="Nom"
                  className="rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-violet-400 focus:ring-2 focus:ring-violet-500/10 outline-none" />
                <input type="text" value={form.site_concerne_email} onChange={e => setForm(f => ({ ...f, site_concerne_email: e.target.value }))} placeholder="Email"
                  className="rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-violet-400 focus:ring-2 focus:ring-violet-500/10 outline-none" />
                <input type="text" value={form.site_concerne_adresse} onChange={e => setForm(f => ({ ...f, site_concerne_adresse: e.target.value }))} placeholder="Adresse"
                  className="rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-violet-400 focus:ring-2 focus:ring-violet-500/10 outline-none" />
                <div className="flex gap-2">
                  <input type="text" value={form.site_concerne_cp} onChange={e => setForm(f => ({ ...f, site_concerne_cp: e.target.value }))} placeholder="CP"
                    className="w-24 rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-violet-400 focus:ring-2 focus:ring-violet-500/10 outline-none" />
                  <input type="text" value={form.site_concerne_ville} onChange={e => setForm(f => ({ ...f, site_concerne_ville: e.target.value }))} placeholder="Ville"
                    className="flex-1 rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-violet-400 focus:ring-2 focus:ring-violet-500/10 outline-none" />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                      ...
                      <tr key={i} className="bg-gray-50/50">
                        <td className="px-2 py-2 text-center">
                          <div className="flex flex-col gap-0.5">
                            <button onClick={() => moveLigne(i, -1)} className="text-gray-300 hover:text-gray-500 cursor-pointer"><svg className="h-3 w-3" viewBox="0 0 24 24" fill="none" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m4.5 15.75 7.5-7.5 7.5 7.5" /></svg></button>
                            <button onClick={() => moveLigne(i, 1)} className="text-gray-300 hover:text-gray-500 cursor-pointer"><svg className="h-3 w-3" viewBox="0 0 24 24" fill="none" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="m19.5 8.25-7.5 7.5-7.5-7.5" /></svg></button>
                          </div>
                        </td>
                    ...
                    return (
                      <tr key={i} className="border-b border-gray-50 hover:bg-gray-50/30">
                        ...
                        <td className="px-2 py-2">
                          ...
                          <input type="text" value={l.description} onChange={e => updateLigne(i, 'description', e.target.value)} placeholder="Description (optionnel)" className="w-full text-xs text-gray-400 border border-gray-100 rounded-lg px-2 py-1 focus:border-violet-400 outline-none" />
                        </td>
            ...
            <div className="flex items-center gap-2 px-5 py-3 border-t border-gray-100 bg-gray-50/50">
              ...
              <button onClick={() => addLigne('COMMENTAIRE')} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-100 text-gray-600 text-xs font-semibold hover:bg-gray-200 transition cursor-pointer">Commentaire</button>
              <button onClick={() => addLigne('SOUS_TOTAL')} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-100 text-gray-600 text-xs font-semibold hover:bg-gray-200 transition cursor-pointer">Sous-total</button>
              <button onClick={() => addLigne('SAUT_DE_LIGNE')} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gray-100 text-gray-600 text-xs font-semibold hover:bg-gray-200 transition cursor-pointer">Séparateur</button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">FTC (Frais techniques)</label>
                <input type="number" step="0.01" value={form.frais_techniques} onChange={e => setForm(f => ({ ...f, frais_techniques: e.target.value }))}
                  className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-violet-400 focus:ring-2 focus:ring-violet-500/10 outline-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">ECT (Eco-contribution)</label>
                <input type="number" step="0.01" value={form.eco_contribution} onChange={e => setForm(f => ({ ...f, eco_contribution: e.target.value }))}
                  className="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:border-violet-400 focus:ring-2 focus:ring-violet-500/10 outline-none" />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <textarea rows={3} value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} placeholder="Notes internes (non affichées sur le PDF)..."
        ...
        <div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 sticky top-24">
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm"><span className="text-gray-500">Total HT</span><span className="font-semibold text-gray-900">{fmt(totalHt)} €</span></div>
              ...
              <div className="border-t border-gray-200 pt-2 flex justify-between text-lg font-bold text-violet-700">
            ...
            <div className="mt-6 space-y-3">
              <button onClick={() => handleSave(false)} disabled={saving}
                className="w-full px-5 py-3 rounded-xl border border-gray-200 text-sm font-semibold text-gray-700 hover:bg-gray-50 disabled:opacity-50 transition cursor-pointer">
                {saving ? 'Enregistrement...' : 'Enregistrer comme brouillon'}
              </button>
              <button onClick={() => handleSave(true)} disabled={saving}
                className="w-full px-5 py-3 rounded-xl bg-violet-600 text-white text-sm font-semibold hover:bg-violet-700 disabled:opacity-50 transition cursor-pointer">
