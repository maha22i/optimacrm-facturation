...
export default function GenerationAbonnementTab({ typeContrat, config }: Props) {
  ...
  const chargerContrats = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get<ApiResponse<ContratsResponse>>(`/factures/contrats-abonnement-a-facturer?date=${dateFacturation}&type=${typeContrat}`);
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">Factures créées en brouillon</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-5">
          <div className="bg-emerald-50 rounded-xl p-4 border border-emerald-100">
            ...
            <p className="text-xs text-emerald-600 font-medium mt-1">Factures créées</p>
          </div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => { setRapport(null); chargerContrats(); }}
          ...
          <div className="flex items-center gap-3">
            ...
            {selection.length > 0 && (
              <span className="text-sm text-gray-500">
                {selection.length} contrat(s) — <span className="font-semibold text-gray-700">{fmt(selectedTotal)} € HT</span>
              </span>
            )}
          </div>
          <button
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex flex-wrap gap-1">
                      {contrat.rubriques.map((r, i) => (
                        <span key={i} className={`text-[10px] font-semibold px-1.5 py-0.5 rounded ${RUBRIQUE_COLORS[r] || 'bg-gray-100 text-gray-700'}`}>{r}</span>
                      ))}
                    </div>
                  </td>
                  <td className="px-4 py-3.5 text-right text-sm text-gray-600">{fmt(contrat.montant_abonnement_ht)} €</td>
                  <td className="px-4 py-3.5 text-right text-sm text-gray-500">{contrat.ftc > 0 ? `${fmt(contrat.ftc)} €` : '—'}</td>
                  <td className="px-4 py-3.5 text-right text-sm font-semibold text-gray-900">{fmt(contrat.total_ht)} €</td>
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6">
            ...
            <p className="text-xs text-gray-400 mb-5">Les factures seront créées en statut <strong>Brouillon</strong>.</p>
            <div className="flex items-center gap-3">
              ...
              <button onClick={lancerGeneration}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[85vh] overflow-hidden flex flex-col">
            <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between">
              <div>
                <h3 className="text-lg font-bold text-gray-900">Aperçu facture</h3>
