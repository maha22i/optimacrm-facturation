...
/**
 * Lecture pure par token — aucune écriture, aucun contexte tenant requis.
 * Repose sur l'escape clause de la policy RLS sur `devis` : le token est un
 * secret unique et non-devinable, il n'y a pas d'ambiguïté tenant à lever
 * pour retrouver la bonne ligne à ce stade.
 */
async function chargerDevisParToken(token) {
...
/**
 * Applique les règles d'accès communes (expiration, statut).
 * `strict` = true pour les actions (seul ENVOYE est accepté).
 *
 * IMPORTANT : doit être appelée à l'intérieur de runWithTenantContext, car
 * elle peut écrire (marquerDevisExpire) — cette écriture doit être
 * RLS-correcte, pas seulement couverte par l'escape clause.
 */
async function appliquerReglesAcces(devis, { strict = true } = {}) {
...
export async function getDevisPublic(req, res, next) {
  try {
    const devis = await chargerDevisParToken(req.params.token);
    ...
    await runWithTenantContext(devis.tenant_id, async () => {
      const validite = toDateStr(devis.date_validite);
      if (devis.statut === 'ENVOYE' && validite && validite < todayUTC()) {
        await devisService.marquerDevisExpire(devis.id);
        devis.statut = 'EXPIRE';
      ...
      const lignesBrutes = await devisService.getLignesDevis(devis.id);
      const societeRes = await query('SELECT raison_sociale, logo_url, adresse_ligne1, adresse_ligne2, code_postal, ville, telephone, email_contact, site_web FROM societe_config LIMIT 1');
...
export async function demanderCode(req, res, next) {
  try {
    ...
    await runWithTenantContext(devis.tenant_id, async () => {
      await appliquerReglesAcces(devis);
